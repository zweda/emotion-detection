...
SOLUTION_PATH = ""  # Output file that will be generated. This file can be directly submitted.
...
def log(line, args=None, close=False):
    ...
    global LOGGER
    if LOGGER is None:
        logfile = 'baseline-EP%d_LR%de-5_LDim%d_BS%d.txt' % (NUM_EPOCHS, int(LEARNING_RATE * (10 ** 5)), LSTM_DIM, BATCH_SIZE)
...
def predictAndSave(model, testSequences, tokenizer=None, load=False):
    if load and tokenizer:
        model = tf.keras.models.load_model(os.path.join(sys.path[0], SAVED_MODELS_DIR, 'baseline_EP%d_LR%de-5_LDim%d_BS%d.h5' % (NUM_EPOCHS, int(LEARNING_RATE * (10 ** 5)), LSTM_DIM, BATCH_SIZE)))
...
def main():
    ...
    global TRAIN_DATA_PATH, TEST_DATA_PATH, SOLUTION_PATH, GLOVE_DIR
    ...
    model.save(os.path.join(sys.path[0], SAVED_MODELS_DIR, 'baseline_EP%d_LR%de-5_LDim%d_BS%d.h5' % (NUM_EPOCHS, int(LEARNING_RATE * (10 ** 5)), LSTM_DIM, BATCH_SIZE)))
